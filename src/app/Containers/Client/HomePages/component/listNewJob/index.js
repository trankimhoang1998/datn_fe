...
import { Avatar, Badge, Button, Col, Drawer, Row, Skeleton, Tag } from "antd";
...
import {
  applyJobRequest,
  fetchListRecruitmentOrderRequest,
...
export default function ListNewJob() {
  const recruitmentorder = useSelector(
    (state) => state.recruitment.listRecruitmentOrder
  );
  ...
  const data = recruitmentorder.result;

  const addCv = () => {
    history.push("/cv");
  };

  const renderListRecruitments = (isCheck) => {
    let jsx = [];
    if (data.length > 0) {
  ...
  return (
    <div className="bg">
      <Container className="job">
        ...
        {data.length > 2 ? renderListRecruitments(checkRole()) : dataNull()}
      </Container>
    </div>
  );
}
